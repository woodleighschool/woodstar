...
- Database: `internal/database` and migrations; focused test helpers: `internal/testutil/{testdb,storagecontract}`
...
- Tests: dependency-free suite `mise run test`; PostgreSQL components `mise run test-postgres`; E2E lifecycles `mise run test-e2e-{munki,osquery,santa,mdp,orbit}` or `mise run test-e2e`; S3 integration `mise run test-integration-storage`
...
`mise run test` requires neither PostgreSQL nor Docker. `mise run test-postgres` and every E2E task use the default local PostgreSQL URL when `WOODSTAR_TEST_DATABASE_URL` is unset; start it explicitly with `docker compose up -d postgres`. Test tasks do not start Compose and tagged dependency lanes do not skip when PostgreSQL or Docker is unavailable. The frontend has no test suite: use web lint/typecheck, generated OpenAPI clients, and the production build.
...
- Dependency-free package tests live beside code as `*_test.go`. Tests that prove PostgreSQL behavior use `*_postgres_test.go` plus `//go:build postgres`; do not use the database merely because the production type owns a handle. Application lifecycles use `//go:build e2e` under `test/e2e`; external provider suites use `//go:build integration` under `test/integration`.
- Assert behavior at the lowest layer that proves it. Keep pure validation and mapping in ordinary tests, fake Woodstar-owned service boundaries, use real PostgreSQL for SQL semantics, and do not mock SQL.
