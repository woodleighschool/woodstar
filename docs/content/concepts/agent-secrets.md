...
Orbit and osquery send the shared enrollment secret once. Woodstar returns a node key for later requests. The other protocol's first enrollment attaches to the same host. Re-enrolling hardware through a protocol that is already attached deletes the existing host and its dependent state before creating a fresh host and node key.
