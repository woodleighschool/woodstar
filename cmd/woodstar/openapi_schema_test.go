...
func TestProtectedOperationsDeclareAuthentication(t *testing.T) {
	...
	if len(session.Delete.Security) != 0 || session.Delete.Responses["401"] != nil {
		t.Fatalf("DELETE session must allow logout without authentication: %#v", session.Delete)
	}
}
