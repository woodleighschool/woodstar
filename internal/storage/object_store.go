...
RETURNING ` + objectColumnsSQL
	obj, err := postgres.GetOne[Object](ctx, s.pool, sql, pgx.NamedArgs{
		"prefix":   prefix,
		"filename": filename,
	})
	if err != nil {
...
WHERE id = $1
  ...
  AND expired_at IS NULL
RETURNING ` + objectColumnsSQL
	object, err := postgres.GetOne[Object](ctx, s.pool, sql, id)
	if err != nil {
		return nil, postgres.MutationError(err)
	}
	return &object, nil
...
RETURNING multipart_upload_id`, id, uploadID).Scan(&recorded)
	...
	if !errors.Is(err, pgx.ErrNoRows) {
		return "", false, postgres.MutationError(err)
	}
...
func (s *ObjectStore) GetByID(ctx context.Context, id int64) (*Object, error) {
	obj, err := postgres.GetOne[Object](ctx, s.pool, objectSelectSQL+"\nWHERE id = $1 AND expired_at IS NULL", id)
	if err != nil {
...
func (s *ObjectStore) ListByPrefix(
	...
	return postgres.ListWithCount[Object](ctx, s.pool, listQuery)
}
...
RETURNING ` + objectColumnsSQL
	object, err := postgres.GetOne[Object](ctx, s.pool, sql, id)
	if err != nil {
		return nil, postgres.DeleteConflict(err, "storage object is still referenced")
	}
	return &object, nil
}
