...
func TestMunkiSoftwareDeploymentAPI(t *testing.T) { //nolint:funlen,cyclop,gocognit // One HTTP contract lifecycle shares one isolated database and router.
	...
	var softwareID, packageID, labelID, hostID, installerObjectID int64
...
INSERT INTO munki_software (
	...
	'Deployment API',
	'com.example.deployment-api',
	'/Applications/Deployment API.app',
	'bundle_short_version'
) RETURNING id`).Scan(&softwareID); err != nil {
		t.Fatalf("insert software: %v", err)
	}
	if err := db.Pool().QueryRow(ctx, `
INSERT INTO storage_objects (prefix, filename)
VALUES ('munki/packages', 'deployment-api-3.0.pkg') RETURNING id`).Scan(&installerObjectID); err != nil {
		t.Fatalf("insert package object: %v", err)
	}
	if err := db.Pool().QueryRow(ctx, `
INSERT INTO munki_packages (software_id, version, installer_type, installer_object_id)
VALUES ($1, '3.0', 'pkg', $2) RETURNING id`, softwareID, installerObjectID).Scan(&packageID); err != nil {
...
VALUES ('deployment-api-host', 'Deployment API Host', 'API-SERIAL') RETURNING id`).Scan(&hostID); err != nil {
	...
	if hostPage.Count != 1 || len(hostPage.Items) != 1 ||
		hostPage.Items[0].Status != munkisoftware.StatusUnknown ||
		hostPage.Items[0].MunkiResult != munkisoftware.MunkiResultNotReported {
		t.Fatalf("assigned hosts = %+v, want unknown installation and no Munki report", hostPage)
	}
	...
	filteredHosts = hostAPIRequest(t, router, http.MethodGet, hostsPath+"?status=unknown", "")
	assertStatus(t, filteredHosts, http.StatusOK, "filter unknown installation status")
	decodeJSON(t, filteredHosts, &hostPage)
	if hostPage.Count != 1 || len(hostPage.Items) != 1 {
		t.Fatalf("unknown installation status hosts = %+v, want one", hostPage)
	}
	assertStatus(
		t,
	...
	if err := inventoryStore.ReplaceHostSoftware(ctx, hostID, []inventory.HostSoftwareEntry{{
		Name:               "Deployment API",
		Version:            "3.0",
		Source:             "apps",
		BundleIdentifier:   "com.example.deployment-api",
		BundleShortVersion: "3.0",
		BundleVersion:      "300",
		InstalledPath:      "/Applications/Deployment API.app",
	}}); err != nil {
		t.Fatalf("replace software inventory: %v", err)
	}
	detail = hostAPIRequest(t, router, http.MethodGet, fmt.Sprintf("/api/munki/software/%d", softwareID), "")
	assertStatus(t, detail, http.StatusOK, "get software after Munki report")
	decodeJSON(t, detail, &detailBody)
	if detailBody.Deployment.AssignedCount != 1 ||
		detailBody.Deployment.ObservedCount != 1 ||
	...
	decodeJSON(t, hosts, &hostPage)
	if hostPage.Count != 1 || hostPage.Items[0].Status != munkisoftware.StatusInstalled ||
		hostPage.Items[0].InstalledVersion != "3.0" ||
		hostPage.Items[0].MunkiResult != munkisoftware.MunkiResultNoInstallNeeded ||
		hostPage.Items[0].TargetVersion != "3.0" ||
		hostPage.Items[0].LastCollectedAt == nil {
		t.Fatalf("assigned host = %+v, want observed installed 3.0 and satisfied Munki result", hostPage)
	}
	filteredHosts = hostAPIRequest(t, router, http.MethodGet, hostsPath+"?status=installed", "")
	assertStatus(t, filteredHosts, http.StatusOK, "filter installed hosts")
	decodeJSON(t, filteredHosts, &hostPage)
	if hostPage.Count != 1 || len(hostPage.Items) != 1 ||
		hostPage.Items[0].Status != munkisoftware.StatusInstalled {
		t.Fatalf("installed hosts = %+v, want one", hostPage)
	}
	filteredHosts = hostAPIRequest(t, router, http.MethodGet, hostsPath+"?munki_result=no_install_needed", "")
	assertStatus(t, filteredHosts, http.StatusOK, "filter Munki result")
	decodeJSON(t, filteredHosts, &hostPage)
	if hostPage.Count != 1 || len(hostPage.Items) != 1 {
		t.Fatalf("no-install-needed hosts = %+v, want one", hostPage)
	}
	...
	if desired.Count != 1 ||
		desired.Items[0].Status != munkisoftware.StatusInstalled ||
		desired.Items[0].InstalledVersion != "3.0" ||
		desired.Items[0].MunkiResult != munkisoftware.MunkiResultNoInstallNeeded ||
		desired.Items[0].TargetVersion != "3.0" ||
		desired.Items[0].LastCollectedAt == nil {
		body, _ := json.Marshal(desired)
		t.Fatalf("host desired software = %s, want the same independent facts", body)
	}
}
