...
type munkiSoftwareHostsInput struct {
	...
	ID          int64                            `path:"id"`
	Status      munkisoftware.InstallationStatus `query:"status,omitempty"`
	MunkiResult munkisoftware.MunkiResult        `query:"munki_result,omitempty"`
...
func (input munkiSoftwareHostsInput) params() munkisoftware.DeploymentHostListParams {
	...
	if input.MunkiResult != "" {
		params.MunkiResult = &input.MunkiResult
	}
	if input.Action != "" {
