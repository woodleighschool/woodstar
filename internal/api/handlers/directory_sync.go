...
func registerDirectorySync(api huma.API, jobs *entra.SyncJobs, logger *slog.Logger) {
	...
	huma.Register(api, huma.Operation{
		...
		if err != nil {
			return nil, handlerError(ctx, logger, "trigger-directory-sync", err)
		}
		return &directorySyncOutput{Body: status}, nil
	})
}
