...
func TestHostListCheckResponseFilter(t *testing.T) {
	...
	passingHost, err := hostStore.UpsertOnOrbitEnroll(ctx, hosts.DetailUpdate{
		HardwareUUID: "host-check-filter-passing",
		OrbitNodeKey: "host-check-filter-passing-orbit",
	})
	if err != nil {
		t.Fatalf("enroll passing host: %v", err)
	}
	failingHost, err := hostStore.UpsertOnOrbitEnroll(ctx, hosts.DetailUpdate{
		HardwareUUID: "host-check-filter-failing",
		OrbitNodeKey: "host-check-filter-failing-orbit",
	})
	if err != nil {
		t.Fatalf("enroll failing host: %v", err)
	}
	unevaluatedHost, err := hostStore.UpsertOnOrbitEnroll(ctx, hosts.DetailUpdate{
		HardwareUUID: "host-check-filter-unevaluated",
	...
	router, cookie := santaAuthedRouter(t, db, "host-check-filter-admin@example.test", func(api huma.API) {
		RegisterHosts(api, hostStore, hosts.NewDeviceMappingStore(db), nil)
	})
	...
	badPaths := []string{
		fmt.Sprintf("/api/hosts?check_id=%d", checkID),
		"/api/hosts?check_response=pass",
		fmt.Sprintf("/api/hosts?check_id=%d&check_response=unknown", checkID),
	}
	for _, path := range badPaths {
		rec := santaAdminRequest(t, router, cookie, http.MethodGet, path, "")
		if rec.Code != http.StatusBadRequest {
			t.Fatalf("%s status = %d, want %d; body = %q", path, rec.Code, http.StatusBadRequest, rec.Body.String())
		}
	}
}
