...
func (s *Store) DeleteMany(ctx context.Context, ids []int64) (int, error) {
	...
	err := pgx.BeginFunc(ctx, s.pool, func(tx pgx.Tx) error {
		...
		if err != nil {
			return postgres.DeleteConflict(err, "Munki package is still referenced")
		}
...
type packageWrite struct {
	...
	InstallerChoicesXML          postgres.JSONSlice[PackageInstallerChoice]              `db:"installer_choices_xml"`
	InstallerEnvironment         postgres.JSONSlice[PackageInstallerEnvironmentVariable] `db:"installer_environment"`
	Installs                     postgres.JSONSlice[PackageInstallItem]                  `db:"installs"`
	Receipts                     postgres.JSONSlice[PackageReceipt]                      `db:"receipts"`
	ItemsToCopy                  postgres.JSONSlice[PackageItemToCopy]                   `db:"items_to_copy"`
