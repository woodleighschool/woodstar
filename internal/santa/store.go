...
const upsertSantaHostObservationSQL = `
...
func (s *Store) UpsertHostObservation(ctx context.Context, observation HostObservation) error {
	...
	_, err := s.db.Pool().Exec(ctx, upsertSantaHostObservationSQL, pgx.NamedArgs{
...
func (s *Store) syncSummary(ctx context.Context, hostID int64) (RuleSyncSummary, error) {
	...
	err := s.db.Pool().QueryRow(ctx, getSantaSyncSummarySQL, hostID).Scan(&desired, &applied, &pending)
