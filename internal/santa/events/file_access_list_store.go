...
func (s *Store) ListFileAccessEvents(
	...
	rows, count, err := postgres.ListWithCount[fileAccessEventRow](
		ctx,
		s.pool,
...
func (s *Store) GetFileAccessEvent(ctx context.Context, id int64) (*FileAccessEvent, error) {
	row, err := postgres.GetOne[fileAccessEventRow](
		ctx,
		s.pool,
...
func fileAccessEventListQuery(params FileAccessEventListParams, where string, args []any) postgres.ListQuery {
...
func fileAccessEventOrderKeys() map[string]postgres.OrderExpr {
...
type fileAccessEventRow struct {
	...
	ProcessChain            postgres.JSONSlice[Process] `db:"process_chain"`
