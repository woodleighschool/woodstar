...
func (s *Store) ListEvents(ctx context.Context, params ExecutionEventListParams) ([]ExecutionEvent, int, error) {
	...
	rows, count, err := postgres.ListWithCount[executionEventRow](
		ctx,
		s.pool,
...
func (s *Store) GetExecutionEvent(ctx context.Context, id int64) (*ExecutionEvent, error) {
	row, err := postgres.GetOne[executionEventRow](ctx, s.pool, executionEventSelectSQL()+"\nWHERE ee.id = $1", id)
	if err != nil {
		return nil, err
	}
...
func addExecutionEventFilters(where *postgres.WhereBuilder, params ExecutionEventListParams) {
...
func executionEventListQuery(params ExecutionEventListParams, where string, args []any) postgres.ListQuery {
...
func eventOrderKeys(eventAlias string, executableAlias string) map[string]postgres.OrderExpr {
	out := map[string]postgres.OrderExpr{
	...
	if executableAlias != "" {
		out["file_name"] = postgres.OrderExpr{SQL: "lower(" + executableAlias + ".file_name)"}
	}
	return out
}
...
type executionEventRow struct {
	...
	SigningChain                postgres.JSONSlice[signingChainEntry] `db:"signing_chain"`
}
