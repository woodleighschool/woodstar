...
func TestRuleStatusesCanonicalizeBundleCollisions(t *testing.T) {
	...
	for pageIndex, wantIdentifier := range []string{firstSHA, secondSHA} {
		statuses, count, err := ruleStore.ListRuleStatusesForHost(
			...
			host.ID,
			configurationID,
			rules.RuleStatusListParams{ListParams: listing.Params{
	...
	if _, _, err := ruleStore.ListRuleStatusesForHost(
		...
		host.ID,
		configurationID,
		rules.RuleStatusListParams{ListParams: listing.Params{PageSize: 1}},
...
func TestBundleRuleExpandsToBinaryHostRules(t *testing.T) {
	...
	if _, err := rules.SyncTargetsFromRules(got); !errors.Is(err, fault.ErrConflict) {
...
func createSantaRuleConfiguration(t *testing.T, db *pgxpool.Pool, name string) int64 {
	t.Helper()
