...
import { LabelChips } from "@features/labels/components/label-chips";
...
function HostPrimaryUserDialog({
  ...
  const form = useForm({
    ...
    validators: {
      onDynamic: z.object({
        email: emailAddress("Enter a valid email or UPN."),
      }),
    },
    onSubmit: async ({ value }) => {
      await setPrimaryUser.mutateAsync({
        id: host.id,
        ...
        >
          ...
          <DialogFooter className="pt-2">
            ...
            <form.Subscribe selector={(state) => state.isSubmitting}>
              {(isSubmitting) => (
                <AsyncButton isPending={pending || isSubmitting} type="submit" size="sm">
                  Save
                </AsyncButton>
              )}
            </form.Subscribe>
