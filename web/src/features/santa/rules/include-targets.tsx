...
import { POLICIES, POLICY_OPTIONS, POLICY_VALUES } from "./metadata";
type DialogState =
  | {
      mode: "add";
    }
            ...
            {(policy) =>
              policy === "cel" ? (
                <form.Field name="cel_expression">
                  {(field) => (
                    <ValidatedFormField field={field} label="Expression" required className="gap-2">
                      {(control) => (
                        <>
                          <CodeEditor
                            value={field.state.value}
