...
export function useUpdateAgentSecret() {
  ...
  return useMutation<AgentSecret, ApiError, { id: number; body: AgentSecretMutation }>({
    mutationFn: ({ id, body }) =>
      unwrap(
        apiClient.PUT("/api/agent-secrets/{agent_secret_id}", { params: { path: { agent_secret_id: id } }, body }),
      ),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.agentSecrets });
    },
...
export function useDeleteAgentSecret() {
  ...
  return useMutation<void, ApiError, number>({
    mutationFn: (id) =>
      unwrap(apiClient.DELETE("/api/agent-secrets/{agent_secret_id}", { params: { path: { agent_secret_id: id } } })),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.agentSecrets });
    },
  });
